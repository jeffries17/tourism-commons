#!/usr/bin/env python3
"""
Automated Assessment for All Tour Operators
Runs Google Search + Website Scraping on all TO Assessment stakeholders
"""
...
def evaluate_website_criteria(html_content: str, url: str) -> dict:
    ...
    if not html_content:
        return {f'WEB{i}': 0 for i in [1,2,3,4,5,6,7,9,10]}
...
def get_tour_operators():
    """Get list of tour operators from TO Assessment sheet"""
    result = service.spreadsheets().values().get(
        spreadsheetId=SHEET_ID,
        range='TO Assessment!A2:A50'  # Get first 50 rows
    ).execute()
...
def write_results_to_sheet(results, start_row=4):
    """Write assessment results to Checklist Detail sheet"""
    print(f"\n💾 Writing results to Checklist Detail (starting row {start_row})...")
...
def main():
    print("=" * 80)
    print("AUTOMATED ASSESSMENT: ALL TOUR OPERATORS")
    print("=" * 80)
    print()
    ...
    # Get tour operators
    print("📋 Getting tour operators from TO Assessment...")
    tour_operators = get_tour_operators()
    print(f"Found {len(tour_operators)} tour operators")
    
    # Run assessments
    print(f"\n🚀 Running automated assessment on {len(tour_operators)} tour operators...")
    results = []
    ...
    total_points = sum(r['total_automated'] for r in results)
    avg_points = total_points / len(results) if results else 0
    high_confidence = sum(1 for r in results if r['confidence'] == 'High')
    
    print(f"Tour Operators Assessed: {len(results)}")
    print(f"Total Points Found: {total_points}")
    print(f"Average Points per TO: {avg_points:.1f}")
    print(f"High Confidence Results: {high_confidence}")
    print()
    
    print("Top Performers:")
    sorted_results = sorted(results, key=lambda x: x['total_automated'], reverse=True)
    for i, result in enumerate(sorted_results[:5], 1):
        print(f"  {i}. {result['stakeholder_name']}: {result['total_automated']}/20 points")
    
    print()
    print("✅ All results written to Checklist Detail!")
