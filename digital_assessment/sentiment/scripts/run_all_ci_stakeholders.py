#!/usr/bin/env python3
"""
Run Corrected Assessment on All Creative Industries Stakeholders
Uses the fixed data format for CI Assessment sheet
"""
...
def write_single_result_corrected(result, row_num):
    ...
    row_data = [
        ...
        'Creative Industries',  # B - Changed from Tour Operator
...
def get_ci_stakeholders():
    """Get list of Creative Industries stakeholders from CI Assessment sheet"""
    result = service.spreadsheets().values().get(
        spreadsheetId=SHEET_ID,
        range='CI Assessment!A2:A100'  # Get first 100 rows
    ).execute()
...
def main():
    print("=" * 80)
    print("CORRECTED ASSESSMENT: ALL CREATIVE INDUSTRIES STAKEHOLDERS")
    print("=" * 80)
    print()
    ...
    # Get CI stakeholders
    print("📋 Getting Creative Industries stakeholders from CI Assessment...")
    stakeholders = get_ci_stakeholders()
    print(f"Found {len(stakeholders)} Creative Industries stakeholders")
    
    if not stakeholders:
        print("❌ No stakeholders found in CI Assessment sheet!")
        return
    
    # Process each one with rate limiting
    print(f"\n🚀 Processing Creative Industries stakeholders with corrected data format...")
    
    for i, stakeholder in enumerate(stakeholders, 1):
        print(f"\n[{i}/{len(stakeholders)}] Processing: {stakeholder['name']}")
        
        try:
            result = automated_assessment(stakeholder['name'])
            write_single_result_corrected(result, 27 + i)  # Start from row 28 (after tour operators)
            ...
            write_single_result_corrected(empty_result, 27 + i)
    
    print("\n" + "=" * 80)
    print("🎉 ALL CREATIVE INDUSTRIES STAKEHOLDERS COMPLETE!")
    print("=" * 80)
    print()
    print("✅ All Creative Industries stakeholders have been assessed")
    print("✅ All data written to Checklist Detail")
    print("✅ CI Assessment should be auto-updating")
    print()
