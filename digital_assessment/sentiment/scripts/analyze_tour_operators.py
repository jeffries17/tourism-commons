#!/usr/bin/env python3
"""
Tour Operators Sentiment Analysis
Analyzes only the tour operators stakeholders separately
"""
...
class TourOperatorsAnalyzer:
    ...
    def analyze_sector_insights(self, all_stakeholder_data: List[Dict]) -> Dict:
        """Analyze insights across the tour operators sector"""
    ...
    def generate_dashboard_summary(self, stakeholder_data: List[Dict], sector_insights: Dict) -> str:
        ...
        summary = f"\n🚌 TOUR OPERATORS SENTIMENT ANALYSIS DASHBOARD\n"
        summary += "=" * 60 + "\n\n"
        ...
        summary += f"🎯 THEME PERFORMANCE ACROSS TOUR OPERATORS:\n"
    ...
    def run_tour_operators_analysis(self):
        """Run the complete tour operators analysis"""
        print("🚌 Starting Tour Operators Sentiment Analysis")
        print("=" * 60)
        
        # Find all English review files in tour operators
        pattern = "../data/sentiment_data/raw_reviews/oct_2025/gambia/tour_operators/**/*_reviews_ENG.json"
        files = glob.glob(pattern, recursive=True)
        
        if not files:
            print("❌ No English review files found in tour operators")
            return
        
        print(f"📁 Found {len(files)} tour operator files to analyze\n")
        ...
        output_file = "../output/tour_operators_sentiment_analysis_results.json"
        ...
        print(f"\n💾 Tour operators analysis saved to: {output_file}")
    ...
    def generate_google_sheets_data(self, stakeholder_data: List[Dict], sector_insights: Dict):
        ...
        # Save CSV
        import csv
        csv_file = "tour_operators_sentiment_analysis_google_sheets.csv"
        ...
        print(f"📊 Tour operators Google Sheets data created: {csv_file}")

def main():
    analyzer = TourOperatorsAnalyzer()
    analyzer.run_tour_operators_analysis()
