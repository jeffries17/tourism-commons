...
async function readMaster(sheetId, range) {
...
async function readSurveyScores(sheetId) {
    const surveyMap = new Map();
    try {
        const rows = await readMaster(sheetId, 'Survey_Scoring');
        if (rows.length < 2)
            return surveyMap; // No data (just header or empty)
        // Skip header row
...
async function readAuthSheet(sheetId) {
    const sheets = await getSheetsClient();
    ...
    return rows
        ...
        role: (row[2] || 'participant').toLowerCase(),
        organizationName: row[0] || '',
        loginCount: parseInt(row[4] || '0', 10),
        lastLogin: row[5] || ''
    }));
}
async function updateLoginCount(sheetId, username) {
...
export function createApp() {
    ...
    app.post('/auth/login', async (req, res) => {
        try {
            ...
            updateLoginCount(sheetId, username).catch(err => console.error('Failed to update login count:', err));
            // Return user data
            res.json({
                user: {
    ...
    app.get('/tour-operators', async (_req, res) => {
        try {
            ...
            const namePattern = /(\btour\b|\btravel\b|\bsafari\b|\bexcursion\b|\bexpeditions?\b|\bguide\b)/i;
            const filtered = allAssessments.filter(p => ((p.sector || '').toLowerCase().includes('tour') || namePattern.test(p.name || '')));
            res.json(filtered);
        }
        catch (e) {
            res.status(500).json({ error: e.message || String(e) });
    ...
    app.get('/participant/plan', async (req, res) => {
        try {
            ...
            const getMarketLabel = (score) => {
                if (score >= 8)
                    return 'Highly Tourist-Focused';
                if (score >= 6)
                    return 'Tourist-Oriented';
                if (score >= 4)
                    return 'Balanced Market';
                if (score >= 2)
                    return 'Local-Oriented';
    ...
    app.get('/participant/justifications', async (req, res) => {
        try {
            ...
            let rowsTourism = [];
            try {
                rowsTourism = await readMaster(sheetId, 'Tourism Assessment');
            }
            catch {
                rowsTourism = [];
            }
    ...
    app.get('/participant/presence', async (req, res) => {
        try {
            ...
            if (idx.website < 0)
                idx.website = 29; // Column AD
            if (idx.facebook < 0)
                idx.facebook = 30; // Column AE  
            if (idx.instagram < 0)
                idx.instagram = 31; // Column AF
            if (idx.tripadvisor < 0)
                idx.tripadvisor = 32; // Column AG
            if (idx.youtube < 0)
                idx.youtube = 33; // Column AH
            if (idx.facebookFollowers < 0)
                idx.facebookFollowers = 34; // Column AI
            if (idx.instagramFollowers < 0)
                idx.instagramFollowers = 35; // Column AJ
            if (idx.tripadvisorReviews < 0)
                idx.tripadvisorReviews = 36; // Column AK
            if (idx.youtubeSubscribers < 0)
                idx.youtubeSubscribers = 37; // Column AL
            if (idx.tiktok < 0)
                idx.tiktok = 38; // Column AM
            if (idx.tiktokFollowers < 0)
                idx.tiktokFollowers = 39; // Column AN
    ...
    app.get('/participant/guidance', async (req, res) => {
        try {
            const name = (req.query.name || '').toString().trim();
            if (!name)
                return res.status(400).json({ error: 'Missing name' });
    ...
    app.get('/participant/opportunities', async (req, res) => {
        try {
            ...
            const recRow = recsRows.find((row, index) => index > 0 && String(row[0] || '').trim().toLowerCase() === name.trim().toLowerCase());
            if (!recRow) {
    ...
    app.post('/update_scores', async (req, res) => {
        try {
            ...
            const columnIndex = categoryColumns[category] + criterionNum - 1;
    ...
    app.get('/sector/:sectorName/baseline', async (req, res) => {
        try {
            ...
            const platformsPerStakeholder = sectorParticipants.map(p => {
                let count = 0;
                if (p.websiteUrl && p.websiteUrl.length > 0)
                    count++;
                if (p.facebookUrl && p.facebookUrl.length > 0)
                    count++;
                if (p.instagramUrl && p.instagramUrl.length > 0)
                    count++;
                if (p.tripadvisorUrl && p.tripadvisorUrl.length > 0)
                    count++;
                if (p.youtubeUrl && p.youtubeUrl.length > 0)
                    count++;
                if (p.tiktokUrl && p.tiktokUrl.length > 0)
                    count++;
                return count;
            });
    ...
    app.get('/platform-adoption/overall', async (_req, res) => {
        try {
            const sheetId = requireEnv('SHEET_ID');
            const sheets = await getSheetsClient();
            const result = await sheets.spreadsheets.values.get({
    ...
    app.get('/platform-adoption/by-sector', async (_req, res) => {
        try {
            const sheetId = requireEnv('SHEET_ID');
            const sheets = await getSheetsClient();
            const result = await sheets.spreadsheets.values.get({
    ...
    app.get('/technical-audit/summary', async (_req, res) => {
        try {
            const sheetId = requireEnv('SHEET_ID');
            const sheets = await getSheetsClient();
            const result = await sheets.spreadsheets.values.get({
    ...
    app.get('/technical-audit/:stakeholderName', async (req, res) => {
        try {
    ...
    app.get('/technical-audit', async (_req, res) => {
        try {
            const sheetId = requireEnv('SHEET_ID');
            const sheets = await getSheetsClient();
            const result = await sheets.spreadsheets.values.get({
 ...
 */
async function generateCheckboxGuidance(name, sector) {
    try {
        const sheetId = requireEnv('SHEET_ID');
        const sheets = await getSheetsClient();
        // Read Checklist Detail sheet
...
function getActionForCriterion(category, index, sector) {
...
function getBenefitForCriterion(category, index, sector) {
    const isTourism = sector.toLowerCase().includes('tour operator');
    const benefits = {
        socialMedia: [
...
function getEffortForCriterion(category, index) {
...
function getTimeframeForCriterion(category, index) {
    const timeframes = {
...
function getCostForCriterion(category, index) {
    const costs = {
...
function mapRowToAssessment(row, sheetType = 'CI') {
