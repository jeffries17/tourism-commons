...
async function readMaster(sheetId: string, range = 'Master Assessment!A1:AI10000') {
...
export function createApp() {
  ...
  app.get('/participant/plan', async (req: Request, res: Response) => {
    try {
      ...
      const rowsMaster = await readMaster(sheetId, 'Master Assessment!A1:AI10000');
      let rowsTourism: string[][] = [];
      try { rowsTourism = await readMaster(sheetId, 'Tourism Assessment!A1:AI10000'); } catch { rowsTourism = []; }
  ...
  app.get('/participant/sector-context', async (req: Request, res: Response) => {
    try {
      ...
      let rowsTourism: string[][] = [];
      try { rowsTourism = await readMaster(sheetId, 'Tourism Assessment!A1:AD10000'); } catch { rowsTourism = []; }
      const rows = rowsMaster.concat(rowsTourism.length ? rowsTourism : []);
  ...
  app.get('/participant/opportunities', async (req: Request, res: Response) => {
    try {
      ...
      const customOpportunities: Array<{ category: string; emoji: string; advice: string; type: string }> = [];
